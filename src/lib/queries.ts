...
const sendInvitationEmail = async (email: string, role: Role) => {
  const apiKey = process.env.RESEND_API_KEY
  const from = process.env.RESEND_FROM_EMAIL
  if (!apiKey || !from) return false

  const appUrl = (process.env.NEXTAUTH_URL ?? process.env.NEXT_PUBLIC_URL ?? '').replace(
    /\/$/,
    ''
  )
  const signUpUrl = `${appUrl}/agency/sign-up`
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      html: `<p>You have been invited to join an agency as <strong>${role}</strong>.</p><p>Complete your account setup here:</p><p><a href="${signUpUrl}">${signUpUrl}</a></p>`,
    }),
  })
  ...
  if (invitationExists) {
    const userDetails = await db.user.upsert({
      ...
      create: {
        ...
        name:
          session.user.name ??
          invitationExists.email.split('@')[0] ??
          'Team Member',
        role: invitationExists.role,
      },
    })
...
export const sendInvitation = async (
  ...
  if (isEmailProviderConfigured) {
    const isSent = await sendInvitationEmail(email, role)
    if (!isSent) {
